...
int main(int argc, char ** argv)
{
    ...
    QGuiApplication::setOrganizationName("Sciton");
    QGuiApplication::setOrganizationDomain("sciton.com");
    QGuiApplication::setApplicationName("Qml Scene");
...
//        QOpenGLContextPrivate::setGlobalShareContext(shareContext.data());
    ...
    if (!options.file.isEmpty()) {
        if (!options.versionDetection || checkVersion(options.file)) {
            ...
            Beep *m_beep = new Beep();
            QObject::connect(&engine, SIGNAL(quit()), QCoreApplication::instance(), SLOT(quit()));
            engine.rootContext()->setContextProperty("beeper", m_beep);
